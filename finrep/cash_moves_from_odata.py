...
def _doc_date(v):
    """1С-дата → «дд.мм.РРРР г:хв:сек» — рівно так, як пише odata_ingest._dt_str.

    Формат має збігатись ДО СИМВОЛА: рядок угоди («Угода 00000002 від 09.10.2025
    14:01:16») працює як ключ зв'язки рухів з deals_all.csv і supplier_invoices.csv.
    День і місяць з нулем попереду, година — без. Розбіжність тут не дала б помилки,
    а тихо розірвала б зв'язку рухів з угодами.
    """
    s = str(v or "")[:19]
    if len(s) < 19:
        return ""
    return "%s.%s.%s %d:%s:%s" % (s[8:10], s[5:7], s[0:4], int(s[11:13]), s[14:16], s[17:19])

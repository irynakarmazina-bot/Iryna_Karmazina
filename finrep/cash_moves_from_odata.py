...
def _doc_date(v):
    """1С-дата → рядок дати документа. Формат бере САМ конвеєр (odata_ingest._dt_str).

    Не переписуємо формат своїми руками: рядок угоди («Угода 00000002 від 09.10.2025
    14:01:16») працює ключем зв'язки рухів з deals_all.csv і supplier_invoices.csv,
    а їх пише odata_ingest. Розбіжність в один символ не дала б помилки — вона тихо
    розірвала б зв'язку. Перша спроба саме так і зламалась: я обрізала нуль у годині
    («8:49:27» замість «08:49:27») і 49 угод із 250 перестали знаходитись.
    Тому беремо ту саму функцію, а власна копія — лише запасний варіант.
    """
    try:
        sys.path.insert(0, os.path.join(BASE, "engine"))
        from odata_ingest import _dt_str  # noqa: PLC0415
        return _dt_str(str(v or ""))
    except Exception:  # noqa: BLE001
        s = str(v or "")
        if not s or s[:10] == "0001-01-01":
            return ""
        d, _, t = s[:19].partition("T")
        y, m, day = d.split("-")
        return ("%s.%s.%s %s" % (day, m, y, t)) if t else ("%s.%s.%s" % (day, m, y))

...
Формули (усі суми в УО = USD-еквівалент, як в Експедиторі):
    ...
    витрати     = Σ витратних рахунків угоди зі статусом «Оплачен» (фактично оплачені)
...
# уточнення: «рахунок виставлений з ЮФ ТА сплачений з ЮФ» — умови ДВІ, обидві одночасно.
# Перевірено 25.08.2026: у закритих угодах ВСІ рахунки виставлені від «Юнітекс Ейч-Ді»,
# організації «Юнітекс Форвардинг» серед них немає жодного разу — тож зараз під це
# правило не підпадає нічого. Раніше я виключала за одним лише рахунком оплати
# і помилково відкидала 7 911 УО справжніх витрат Ейч-Ді.
EXCLUDE_ORG = "Юнітекс Форвардинг"
...
def collect(c):
    ...
    org = names(c, "Catalog_Организации")
    ...
    for r in page(c, "Document_РасходнаяНакладная",
        ...
        if dk not in deals or not r.get("Posted"):
            continue
        d = deals[dk]
        if (org.get(r.get("Организация_Key")) == EXCLUDE_ORG
                and acc.get(r.get("ВидОплаты_Key")) in EXCLUDE_EXPENSE_ACCOUNTS):

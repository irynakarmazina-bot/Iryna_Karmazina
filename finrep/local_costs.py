...
Формули (усі суми в УО = USD-еквівалент, як в Експедиторі):
    ...
    витрати     = Σ витратних рахунків угоди зі статусом «Оплачен» (з будь-якого рахунку)
    ...
    інфо+комісії= сплачені витратні рахунки зі статтями INFO_ARTICLES;
...
def collect(c):
    ...
    for r in page(c, "Document_РасходнаяНакладная",
        ...
        if str(r.get("СтатусСчета") or "") != PAID:
            continue                                      # неоплачене у профіт не входить

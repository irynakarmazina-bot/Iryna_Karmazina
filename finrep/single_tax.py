...
ВАЛЮТА: рахунок гривневий, тому сума береться у ВАЛЮТІ РАХУНКА (грн), а не в У.О.
Рядки рахунка в Експедиторі зберігають суму в У.О. (Сумма_УЕ), тому винагорода
переводиться в гривню тим самим курсом, що й весь рахунок:
        винагорода_грн = винагорода_УО × (СуммаВал рахунка / сумма_УЕ рахунка).
Якщо в рядках знайдеться поле із сумою у валюті — воно береться напряму
(див. --probe, який показує реальний набір полів).
...
def main():
    ...
    for r in lines:
        ...
        if i and r.get("Статья_Key") == fee_g:
            i["fee_uo"] += f(r.get("Сумма_УЕ"))

    rows = []
    for i in inv.values():
        fee = round(i["fee_uo"] * i["rate"], 2)      # винагорода у гривні
        if not fee:
            continue
        rows.append({"deal": i["deal"], "invoice": i["num"], "paid": i["paid"],
                     "fee": fee, "t5": round(fee * RATE_5, 2), "t1": round(fee * RATE_1, 2),

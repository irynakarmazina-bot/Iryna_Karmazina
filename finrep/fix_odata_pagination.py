...
ЩО РОБИТЬ ЦЕЙ СКРИПТ
    ...
    2. Звіряє зібрану кількість з еталоном `/$count`. Якщо зібрано менше — зупиняється
       з повідомленням, у якому вказано об'єкт, скільки зібрано і скільки мало бути.
       Звірка вмикається лише коли тягнемо об'єкт ЦІЛКОМ — з `$filter` або `$top`
       очікувана кількість інша. Якщо `/$count` недоступний, поведінка така сама, як була.

...
# 31.07.2026 користувачка: «Виконується — став Виконується, додай цей статус».
# «ВыставленСчет» так і лишається без мапінгу (окремого рішення по ньому ще немає).
STAGE_MAP = {"Завершена": "Вантаж доставлено", "Выполняется": "Виконується"}

...
GATE_OUT = "Гейт аут"                      # вивіз контейнера з порту прибуття
...
READ_FIELDS = ["Id", "Угода", "Напрямок", "BL", "Контейнер", "Контейнер (лінія)", "ETA",
               ...
               "Вивантаження в порту (факт)", "ETA сухий порт",
...
def parse_events(events, row, today_iso, statuses=frozenset()):
    ...
    if code in ("LOAD", "DEPA"):
        ...
        # вивіз з порту ПРИБУТТЯ на залізничному плечі — «Гейт аут», а не
        # «завантажений на потяг» (саме тут плуталась угода 224)
        st = GATE_OUT if (mode == "RAIL" and not sea_ahead) else load_st
    elif code == "GTIN":

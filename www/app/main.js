...
async function enter(){
  ...
  /* F5 повертає на ту саму сторінку (прохання користувачки 02.09.2026).
     go() сам відкине сторінку, якої немає в nav цієї ролі. */
  let lastPage = "";
  try{ lastPage = sessionStorage.getItem("cur-page") || ""; }catch(e){ /* приватний режим */ }
  go(lastPage || cfg().nav[0]);
...
function go(page){
  if (!cfg().nav.includes(page)) page = cfg().nav[0];
  CUR = page;
  /* Після F5 відкривається ТА САМА сторінка, а не Дашборд (прохання
     користувачки 02.09.2026). Запам'ятовуємо кожен перехід; відновлення —
     при вході (див. виклик go після enter). */
  try{ sessionStorage.setItem("cur-page", page); }catch(e){ /* приватний режим */ }
...
const FLAGS = ["Телекс","Т1","ДО","Документи","SI","Замитнення","Реліз","Закрито нами"];
PAGES.dispatch = async () => {
  ...
  all.sort((a,b)=>{
    ...
    const ph = isPhone();
    /* «Закрито нами» (галочка в картці, 25.08.2026): експортна угода, де вантаж
    ...
    /* ДОСТАВЛЕНІ — за датою вивантаження, найсвіжіша ЗНИЗУ блоку (вимога
       користувачки 02.09.2026: «відсортуй по даті вивантаження — найближча
       знизу»). На комп'ютері блок доставлених стоїть над активними, тож
       найсвіжіша доставка опиняється одразу над першим активним вантажем.
       Дата — за тим самим ланцюжком, що і колонка «Доставлено» у звітах:
       факт у отримувача → планова до клієнта (факт) → вивантаження в порту →
       ETA порт (факт). Без жодної дати — угорі блоку (старі угоди без дат). */
    if (_done(a) && _done(b)){
      const dd = r => {
        for (const f of ["Вивантаження у отримувача (факт)", "Планова до клієнта (факт)",
                         "Вивантаження в порту (факт)", "ETA порт (факт)"]){
          const m = /(\d{4})-(\d{2})-(\d{2})/.exec(String(r[f] || ""));
          if (m) return m[1] + m[2] + m[3];
        }
        return "";
      };
      const da = dd(a), db = dd(b);
      if (da !== db) return da < db ? -1 : 1;   // порожнє ("") — найвище
  ...
  /* ⚠️ КОЖНА дата-колонка, у якої в рядку є data-ed, МУСИТЬ бути тут зі
     значенням "date". Інакше вона редагується як голий текст: без календарика
     і без розбору дати — «31.08» їде в базу як є, і NocoDB перетворює його на
     «1970-01-01» / «Invalid Date». Саме так 02.09.2026 зіпсувалась дата
     «Доставлено» в угоді 238 (колонки «Гейт аут» і «Вивантаження у отримувача»
     додали в таблицю 25.08, а сюди — забули). */
  const ED_KIND = {"ETA":"date", "ETD (факт)":"date", "Гейт ін":"date",
                   ...
                   "Здача в порт (факт)":"date", "Гейт аут":"date",
                   "Вивантаження у отримувача (факт)":"date",
  ...
  /* 90 → 120 і БЕЗ стискання нижче цього мінімуму (вимога користувачки
     31.08.2026: «щоб коментар було видно, най він ховається за край екрана»).
     До того на вузькому екрані гілка «місця бракує» зрізала коментар до нуля —
     в колонці лишалась одна літера «а…». */
  const FLEX_MIN = 120;            // мінімум читабельності — не вужче ніколи
  let colFitKey = "";
  const fitCols = (force, src) => {
    ...
    if (w[FLEX_COL] !== undefined) {
      ...
      /* Місця бракує — коментар НЕ стискаємо: він лишається на FLEX_MIN, а
         таблиця йде в бічну прокрутку. Було: коментар зрізався першим аж до
         нуля, і на вузькому екрані від нього лишалась одна літера. Вимога
         користувачки 31.08.2026: «щоб коментар було видно, най він ховається
         за край екрана». Статус, номери й дати, як і раніше, не чіпаємо
         («статус не може ховатися за край екрана», 05.08.2026). */
    }
  ...
  const editCell = (td) => {
    ...
    /* Втрата фокуса закриває редактор, АЛЕ не тоді, коли фокус перейшов на
       сусідній календарик у цій же клітинці: клік по ньому давав blur → редактор
       зберігався і зникав раніше, ніж відкривався календар — кнопка виглядала
       мертвою (скарга користувачки 25.08.2026). */
    const stillInside = e => e.relatedTarget && td.contains(e.relatedTarget);
    inp.addEventListener("blur", e=>{ if (!stillInside(e)) finish(true); });
    ...
    if (kind === "date"){
      bindDatePair(td.querySelector(".dted"), ()=>finish(true));
      const pick = td.querySelector(".edpick");
      if (pick) pick.addEventListener("blur", e=>{ if (!stillInside(e)) finish(true); });
    }
  };
  ...
  const draw = (keep) => {
    ...
    const rowsHtml = bar + (shown.length ? shown.map(r=> logist ? `
      <tr data-id="${r.Id}" class="${(r["Статус"]==='Вантаж доставлено'||r["Закрито нами"])?'done':''}" style="cursor:pointer">
      ...
      <tr data-id="${r.Id}" class="${(r["Статус"]==='Вантаж доставлено'||r["Закрито нами"])?'done':''} ${truckLate(r)?'alertrow':((stageStale(r)||gateInMissing(r)||staleStatus(r)||trackSilent(r))?'warnrow':'')}" style="cursor:pointer"
...
const CARD_GROUPS = [
  ...
  ["Море", ["Судно","Вояж","Контейнер (лінія)","Звірка","Статус","Закрито нами","Етап (Експедитор)","ETA","ETD (план)","ETD (факт)","ETA порт (план)","ETA порт (факт)","Порт перевалки","Перевалка (прибуття)","Перевалка (відправлення)","Зміни ETA (історія)","Остання зміна","Останнє оновлення","Статус (джерело)","Статус (оновлено)","Трекінг (стан)"]],
...
const CARD_LABEL = {"Коментар клієнту":"Коментар для клієнта",
                    ...
                    "Закрито нами":"Закрито з нашого боку"};
const CARD_NOTE  = {"Коментар клієнту":"це поле клієнт бачить у своєму кабінеті",
                    "Контейнер (лінія)":"⚠ лінія бачить ІНШИЙ номер, ніж стоїть в угоді — звірте",
                    "Закрито нами":"вантаж у морі, реліз віддано, наше сплачено — рядок ховається до доставлених, статус НЕ міняється",
...
function openRow(r){
  ...
  $("row-body").innerHTML = groups.map(([g,fields])=>{
    /* «Контейнер (лінія)» — це номер, який бачить трекінг лінії; він потрібен
       для ЗВІРКИ. Коли він збігається з «Контейнер» (або порожній), рядок був
       чистим дублем — «тут дублюється контейнер», користувачка 25.08.2026.
       Тепер рядок малюється ЛИШЕ при розбіжності — як сигнал, що в угоді та в
       лінії стоять різні номери. */
    const _cn = v => String(v || "").toUpperCase().split(/[,\s]+/).filter(Boolean).sort().join(",");
    const items = fields.filter(f => f !== "Контейнер (лінія)"
        || (_cn(r[f]) && _cn(r[f]) !== _cn(r["Контейнер"]))).map(f=>{
...
function editCardField(td, r){
  ...
  // той самий захист календарика, що в таблиці (див. editCell)
  const stillInside = e => e.relatedTarget && td.contains(e.relatedTarget);
  inp.addEventListener("blur", e=>{ if (!stillInside(e)) finish(true); });
  inp.addEventListener("keydown", e=>{
...
PAGES.accounting = async () => {
  ...
  <div class="card">
    ...
    <p class="sub">закриті угоди з галочкою <b>«Оброблена (нарахований прибуток)»</b>,
      де рахунок клієнту сплачений на <b>Юнітекс Ейч-Ді</b> ·
      рахунки й витрати з видом оплати <b>Банк Юнітекс Форвардинг</b> не враховуються ·
    ...
    <div class="note" style="margin-top:12px">
      <b>Як рахується:</b> беремо закриту угоду, у якій стоїть галочка
      «Оброблена (нарахований прибуток)». <b>Залишок</b> = дохід угоди мінус оплачені
      витрати — та сама цифра, що в шапці угоди в Експедиторі.
      Винагорода = рядки тих самих доходних рахунків зі статтею «Винагорода експедитора».
      ${(d.info_articles||[]).length
        ? `До залишку <b>додаються назад</b> банківські комісії та податки
           (${d.info_articles.map(esc).join(", ")}) — це операційні витрати компанії,
           вони не мають зменшувати суму до переказу.`
        : ``}
      <b>Різниця = залишок + комісії та податки − винагорода експедитора</b>;
      типово показані тільки додатні.
    </div>
  ...
  const COLS = [
    ...
    {h:"Залишок", k:"profit", n:true}, {h:"Комісії та податки", k:"info_added", n:true},

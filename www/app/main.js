...
const CARD_GROUPS = [
  ["Основне", ["Угода","Напрямок","Клієнт","Лінія","Вид перевезення","FCL/LCL","Тип","Умови поставки (Інкотермс)","Перевізник","Агент","Маршрут","POL","POD","FD","Митне оформлення","Кінцева точка доставки","HBL","BL","Контейнер","Вантаж","Тип обладнання","Кількість","Вивіз (Carrier/Merchant)","Сухий порт","Адреса розмитнення"]],
...
const CARD_LOCKED = {"Угода":1,"Клієнт":1,"Напрямок":1,"Лінія":1,"Менеджер":1,"Агент":1,
                     "Контейнер":1,"Етап (Експедитор)":1,"Зміни ETA (історія)":1,
                     ...
                     /* маршрут і його ланки веде Експедитор (03.08.2026): «Маршрут»
                        збирається з POL → POD → DR → FD, правити його в платформі
                        безглуздо — синхронізація перезапише. Міняти треба в угоді. */
                     "Маршрут":1,"POL":1,"POD":1,"FD":1,"Митне оформлення":1};
  ...
  if (FLAGS.includes(f)){
    ...
    saveField(r, f, nv)
      .then(()=>{ td.innerHTML = `<span class="flag ${nv?"on":""}">${nv?"✓ так":"—"}</span>`;
                  toast(`✅ ${f}: ${nv?"так":"ні"}`); DISP_CACHE = DISP_CACHE; })

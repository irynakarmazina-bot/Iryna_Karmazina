...
/* ПОРЯДОК РЯДКІВ. Раніше сортували лише за ETA — і вантаж, який ще навіть не
   вийшов з порту, ставав ВИЩЕ за той, що вже пливе (зауваження користувачки
   14.08.2026: угода 259 з ETD 17.08 стояла над 251, яка в морі з 29.07).
   Тепер головне — ЕТАП: хто далі просунувся, той вище. Доставлені завжди в
   кінці. Всередині одного етапу — за датою прибуття, потім відправлення,
   потім за номером угоди, щоб порядок не «стрибав» при однакових даних.
   Етапи беруться з ST_ORDER — того самого списку, за яким малюється схема
   руху. Невідомий статус дає -1 і йде під усі відомі, а не в випадкове місце. */
const stRank = r => ST_ORDER.indexOf(s(r,"Статус"));
function byProgress(a, b){
  if (done(a) !== done(b)) return done(a) ? 1 : -1;
  const ra = stRank(a), rb = stRank(b);
  if (ra !== rb) return rb - ra;
  const ea = s(a,"ETA") || "9999", eb = s(b,"ETA") || "9999";
  if (ea !== eb) return ea < eb ? -1 : 1;
  const da = s(a,"ETD (факт)") || s(a,"ETD (план)") || "9999";
  const db = s(b,"ETD (факт)") || s(b,"ETD (план)") || "9999";
...
def main():
    ...
    for r in rows:
        ...
        d["_docs"] = files_of(r)
        data.append(d)
    # Базовий, передбачуваний порядок. Остаточний порядок задає браузер
    # (функція byProgress: спершу етап перевезення, потім дати) — список етапів
    # живе в шаблоні, і другої його копії тут бути не має.

...
function steps(r){
  ...
  /* Іконка ГОЛОВНОГО плеча — за самим плечем, а не за «Вид перевезення».
     Було: угода «фрахт+ТЕО+залізниця» малювала вузол «В морі» потягом, бо
     залізниця — це доставка ДО порту, а не сам рейс (02.08.2026). Потяг тепер
     має власний вузол «Завантаження на потяг». */
  const moveI = A ? "plane" : (modeOf(r) === "road" ? "truck" : "ship");

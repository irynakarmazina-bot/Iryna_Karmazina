...
CLIENT_COLS = [
    ...
    "Кінцева точка доставки", "ПОО", "POL", "POD", "Планова до клієнта (план)",
...
/* Три дати наземного плеча — окремими колонками в таблиці, щоб їх було видно
   без розкриття угоди (вимога користувачки 21.08.2026).
   «Завантажено» — коли поставили на потяг або на авто: «Гейт аут» це і є виїзд
   з порту. «Сухий порт» — план прибуття, який залізничний оператор кладе в
   `ETA сухий порт` ПІСЛЯ завантаження; до завантаження там лежить Маерсків план
   самого завантаження, тому до появи факту колонка порожня. «Доставка» — факт
   видачі отримувачу, а поки його немає — планова дата до клієнта. */
const loadOf  = r => s(r,"Подача авто (факт)") || s(r,"Гейт аут") || s(r,"Подача авто (план)");
const dryOf   = r => (s(r,"Подача авто (факт)") || s(r,"Гейт аут")) ? s(r,"ETA сухий порт") : "";
const delivOf = r => s(r,"Вивантаження у отримувача (факт)")
                  || s(r,"Планова до клієнта (факт)") || s(r,"Планова до клієнта (план)");
...
function steps(r){
  ...
  /* Місця беремо з ПОЛІВ, а не з позиції в «Маршруті». Причина (25.08.2026):
     21.08 в маршрут додалася перша ланка ПОО — місце завантаження — і
     позиційний відлік поїхав: «Порт відправлення» показував Солоницівку
     замість Гданська. Так було у 23 угодах, майже всі — експорт ГРАНД МАРИН.
     Якщо ПОО заповнене, а POL/POD — ні, місце порту НЕВІДОМЕ, і брати наступну
     ланку не можна: в угоді 24 «Масса → Львів» другою ланкою стоїть Львів —
     призначення, а не порт відправлення. Краще порожньо, ніж не те місто. */
  const poo  = s(r,"ПОО");
  const from = s(r,"POL") || (poo ? "" : (route[0] || ""));
  const to   = s(r,"POD") || (poo ? "" : (route[1] || ""));
  ...
  if (!imp){
    ...
    /* «Завантаження на авто» і «Стафіровка» в експорті — ОДИН І ТОЙ САМИЙ крок
       (уточнення користувачки 25.08.2026). Раніше це були два вузли поспіль з
       тим самим місцем, і виглядало як дві різні події. Крок називається просто
       «Стафіровка» — назву обрала користувачка 25.08.2026.
       Місце — ПОО, тобто де вантаж завантажили, а не порт. */
    const pre = [
      { k:"stuff", t:"Стафіровка", i:"warehouse",
        d:s(r,"Stuffing") || s(r,"Подача авто (факт)") || s(r,"Подача авто (план)"),
        f:true, p:poo || from },
    ];
    if (byTrainX){
  ...
  const car  = s(r,"Подача авто (факт)") || s(r,"Гейт аут") || s(r,"Подача авто (план)");
  const carF = !!(s(r,"Подача авто (факт)") || s(r,"Гейт аут"));
  /* ОДНЕ поле `ETA сухий порт` означає РІЗНЕ на різних етапах — перевірено на
     угоді 224: 21.08 зранку там було 10.08, а вдень уже 26.08.
     • поки вантаж НЕ завантажений (немає «Гейт аут») — це Маерсків ПЛАН
       завантаження на потяг, тобто дата ЦЬОГО кроку;
     • після завантаження залізничний оператор перезаписує його своїм ПЛАНОМ
       ПРИБУТТЯ В СУХИЙ ПОРТ — тобто датою наступного кроку.
     Розрізняємо за наявністю факту завантаження. */
  const dryEta = s(r,"ETA сухий порт");
  ...
  land.push({ k:"land", t: byTrain ? "Завантажений на потяг" : "Завантажений на авто",
              i: byTrain ? "train" : "truck",
              d: car || (carF ? "" : dryEta), f: carF, p:"" });
  land.push({ k:"border", t:"Кордон", i:"border",
              d:s(r,"На кордоні") || s(r,"Перетин кордону (факт)"), f:true, p:"" });
  /* Сухий порт показуємо ЛИШЕ коли везуть потягом — правило користувачки
     21.08.2026, те саме, що вже діяло в експорті. Було «якщо залізниця АБО
     заповнене поле ETA сухий порт»: у автомобільної угоди з випадково
     заповненим полем малювався зайвий крок. */
  if (byTrain)
    land.push({ k:"dry", t:"Сухий порт", i:"crane",
                d: carF ? dryEta : "", f:false, p:s(r,"Сухий порт") });
  return [
...
const ST_STEP = {
  ...
  "Завантажений на авто":"land", "Завантажений на потяг":"land", "Гейт аут":"land",
...
const ST_ORDER = ["Букінг","Виконується","Стафіровка","В порту відправлення",
  ...
  "Завантажений на авто","Завантажений на потяг","Гейт аут","Вивантажений в сухому порту",
...
function stepState(r){
  ...
  /* В ЕКСПОРТІ «Завантажений на авто/потяг» — це плече ДО порту, і вузла з таким
     ключем у ланцюжку немає. Без цієї підміни підсвітка не знаходила крок узагалі
     і зникала (зауваження користувачки 02.08.2026). Ведемо на «Завантаження та
     стафіровку» — з 25.08.2026 це один вузол, окремого «carauto» більше немає. */
  if (key === "land" && !st.some(x => x.k === "land")) key = "stuff";
  ...
  st.forEach((x, i) => {
    const d = x.d2 || x.d;
    /* ПЛАНОВА дата не є доказом, що етап відбувся, навіть якщо вже минула.
       Угода 224: план прибуття в сухий порт минув, і крапка стрибала на
       «Сухий порт», хоча вантаж ще їде потягом. Рухають лише факти. */

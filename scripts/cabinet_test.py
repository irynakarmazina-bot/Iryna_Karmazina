...
ROWS = [
    ...
    {"Угода": "102", "Клієнт": "мірандор  ", "Статус": "Вантаж доставлено", "Менеджер": "Ірина",
    ...
    {"Угода": "105", "Клієнт": "Мірандор", "Статус": "Вантаж доставлено", "Менеджер": "Ірина",
    ...
    {"Угода": "103", "Клієнт": "Мірандор Плюс", "Статус": "В морі", "Менеджер": "Петро",
...
# Назву колонки обрала користувачка 24.08.2026: «Судно / Лінія».
check("колонка названа «Судно / Лінія»", "Судно / Лінія" in p11)
check("старої назви не лишилось", "Судно / авіалінія" not in p11)
...
print("\n=== 11е. Кабінети клієнтів: хто кого бачить ===")
# Рішення користувачки 24.08.2026: адміністратор бачить усі кабінети,
# сейлз-менеджер — ЛИШЕ своїх клієнтів, фінансист і бухгалтер — жодного.
# Перевіряємо на СЕРВЕРІ: браузер може попросити що завгодно.
FakeGW.role = "Фінансист"
for path in ("/cabinet-clients", "/cabinet-accounts"):
    code, body, _ = get(path)
    check("фінансист не бачить %s" % path, code == 403, code)
FakeGW.role = "Бухгалтер"
code, _, _ = get("/cabinet-clients")
check("бухгалтер не бачить кабінетів", code == 403, code)

FakeGW.role = "Адміністратор"
code, body, _ = get("/cabinet-clients")
adm = {r["client"] for r in json.loads(body)["list"]}
check("адміністратор бачить обидві компанії", {"Мірандор", "Мірандор Плюс"} <= adm, adm)

# ROWS: угоди «Мірандор» веде Ірина, «Мірандор Плюс» — ні (менеджера немає).
class SalesGW:
    name = "Ірина"
    @staticmethod
    def whoami(jwt):
        return ("sales@unitex.od.ua", SalesGW.name, "Сейлз-менеджер")

CAB._GW = SalesGW
code, body, _ = get("/cabinet-clients")
sales = {r["client"] for r in json.loads(body)["list"]}
# У фікстурі назва «Мірандор» навмисно написана двома способами (перевірка
# регістру), тому звіряємо не дослівно: головне — чужої компанії немає.
check("сейлз бачить лише свою компанію",
      "Мірандор Плюс" not in sales and any(s.lower().strip() == "мірандор" for s in sales),
      sales)
code, body, _ = get("/cabinet-accounts")
sacc = {r["client"] for r in json.loads(body)["list"]}
check("і акаунти лише своєї компанії", sacc <= {"Мірандор"}, sacc)
# підміна назви в запиті не має відкривати чужий кабінет
code, _, _ = post("/cabinet-view", {"client": "Мірандор Плюс"}, origin=False)
check("чужу компанію відкрити не можна", code in (403, 404), code)
code, _, _ = post("/cabinet-invite", {"email": "chuzhyi@m.ua"}, origin=False)
check("посилання чужому акаунту не видається", code in (403, 404), code)
# сейлз без імені в довіднику не бачить НІЧОГО (обмежувати нема по чому)
SalesGW.name = ""
code, _, _ = get("/cabinet-clients")
check("сейлз без імені — 403, а не «усі»", code == 403, code)
SalesGW.name = "Ірина"
CAB._GW = FakeGW
FakeGW.role = "Адміністратор"
